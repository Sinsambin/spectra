...
module.exports = {
	name: 'ocr',
    description: 'This command shows the most recent pass in osu!catch the beat mode(in the past 24 hours) about a specific osu! player.\n If you are not registered, you will need to specify a user.',
    cooldown: 10,
	...
	async execute(msg, args, cl, Register) {
        ...
        const query = 'https://osu.ppy.sh/api/get_user_recent?u=' + searchuser + '&k=' + osuapikey + '&limit=50&m=2';
        ...
        if(body.length > 0) {
            ...
            if(foundpass) {
                ...
                const accCalc = (((50 * head.count50) + (100 * head.count100) + (300 * head.count300)) / (300 * (1 * head.countmiss + 1 * head.count50 + 1 * head.count100 + 1 * head.count300))) * 100;
                ...
                const resultEmbed = new Discord.RichEmbed()
                    ...
                    .setDescription('This User didn\'t pass any map recently on osu!catch the beat mode.')
                    .setTimestamp()
                    .setFooter(msg.author.tag);
                    msg.channel.send(resultEmbed);
            }
