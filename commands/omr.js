...
module.exports = {
	name: 'omr',
    description: 'This command shows the most recent pass in osu!mania mode(in the past 24 hours) about a specific osu! player.\n If you are not registered, you will need to specify a user.',
    cooldown: 10,
	...
	async execute(msg, args, cl, Register) {
        ...
        const query = 'https://osu.ppy.sh/api/get_user_recent?u=' + searchuser + '&k=' + osuapikey + '&limit=50&m=3';
        ...
        if(body.length > 0) {
            ...
            if(foundpass) {
				...
				// countkatu = 200, countgeki = 300 Rainbow
                const accCalc = (((50 * head.count50) + (100 * head.count100) + (200 * head.countkatu) + (300 * (1 * head.count300 + 1 * head.countgeki))) / (300 * (1 * head.countmiss + 1 * head.count50 + 1 * head.count100 + 1 * head.countkatu + 1 * head.count300 + 1 * head.countgeki))) * 100;
                const resultEmbed = new Discord.RichEmbed()
                    ...
                    .addBlankField(false)
                    .addField('Hits', `${head.countgeki}/${head.count300}/${head.countkatu}/${head.count100}/${head.count50}/${head.countmiss}`, true)
                ...
                const resultEmbed = new Discord.RichEmbed()
                    ...
                    .setDescription('This User didn\'t pass any map recently in osu!mania mode.')
                    .setTimestamp()
                    .setFooter(msg.author.tag);
                    msg.channel.send(resultEmbed);
            }
